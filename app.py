# ===============================
# app.py - Food Calorie Estimator + Personalized Diet Planner (Render-ready)
# ===============================
...
form.addEventListener('submit', async (e)=>{
    ...
    const res = await fetch(window.location.origin+'/predict/', { method:'POST', body: formData });
...
# ===============================
# 6️⃣ Predict route (Render-ready, in-memory)
# ===============================
@app.post("/predict/")
async def predict_calories(file: UploadFile = File(...)):
    try:
        ...
        # YOLOv8 prediction
        results = model.predict(source=img_rgb, conf=0.35, save=False, verbose=False, imgsz=640, device='cpu')
        result = results[0]
        ...
        for box, cls, conf in zip(result.boxes.xyxy, result.boxes.cls, result.boxes.conf):
            ...
            cv2.putText(img_rgb, class_name, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,165,0), 2)
        ...
        # ensure table works even if no detection
        if len(items) == 0:
            items.append({
                "class": "Unknown",
                "confidence": 0,
                "calories_per_100g": {"calories":0,"protein":0,"carbs":0,"fat":0}
            })
